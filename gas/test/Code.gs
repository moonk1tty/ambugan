/**
 * ==============================================================================
 * SPLITNEST TELEGRAM BOT & GOOGLE SHEETS BACKEND — [TEST ENVIRONMENT]
 * ==============================================================================
 * 
 * Environment: TEST ENVIRONMENT
 * Bot: @splistnest_test_bot
 * 
 * Configured with Test Credentials:
 * • TELEGRAM_BOT_TOKEN: 8975116420:AAG6KT1W-ooFG-zFq4QcEDWAL9U9AotRT2Y
 * • SPREADSHEET_ID: 1w7-vyYvVPO505o6UlbfI29LA8qOWtLpuWP4_lFXLym0
 * • MINI_APP_URL: t.me/splistnest_test_bot/test
 * 
 * Features:
...
// ==============================================================================
var TELEGRAM_BOT_TOKEN = "8975116420:AAG6KT1W-ooFG-zFq4QcEDWAL9U9AotRT2Y";
var SPREADSHEET_ID = "1w7-vyYvVPO505o6UlbfI29LA8qOWtLpuWP4_lFXLym0";
var MINI_APP_URL = "t.me/splistnest_test_bot/test";
...
function getDbSpreadsheet() {
  ...
  if (SPREADSHEET_ID && SPREADSHEET_ID.trim() !== "" && SPREADSHEET_ID !== "YOUR_SPREADSHEET_ID_HERE") {
    try {
      ...
      ss = SpreadsheetApp.getActiveSpreadsheet() || SpreadsheetApp.create("splitnest Expense Database (Test)");
    }
  } else {
    ss = SpreadsheetApp.getActiveSpreadsheet() || SpreadsheetApp.create("splitnest Expense Database (Test)");
  }
...
function doGet(e) {
  try {
    ...
    if (action === "test_message" && cleanChatId) {
      var testRes = sendTelegramMessage(cleanChatId, "🔔 <b>splitnest Test Notification [Test Bot]</b>\n\nYour test Telegram bot is successfully connected!", getAppReplyMarkup(cleanChatId));
...
function handleTelegramUpdate(update) {
  ...
  if (update.my_chat_member) {
    ...
    if (!wasInGroup && isNowInGroup && !alreadyWelcomed) {
      ...
      var welcomeText = "👋 <b>splitnest [Test Bot] joined the group!</b>\n\n" +
  ...
  if (msg.photo && Array.isArray(msg.photo) && msg.photo.length > 0) {
    try {
      ...
      sendTelegramMessage(chatId, "🔍 <i>[Test Bot] Scanning receipt with Gemini AI...</i>");
      ...
      if (fileJson.ok && fileJson.result && fileJson.result.file_path) {
        ...
        if (ocrResult.ok && ocrResult.data) {
          ...
          var ocrMsg = "🧾 <b>Gemini AI Receipt Scanned! [Test Bot]</b>\n" +
                       "━━━━━━━━━━━━━\n" +
  ...
  if (commandOnly === "/start" || commandOnly === "!start") {
    var startReply = "✨ <b>Welcome to splitnest! [Test Bot]</b>\n\n" +
  ...
  if (commandOnly === "/help" || commandOnly === "!help" || lowerText === "help") {
    var helpText = "📖 <b>splitnest Bot Commands [Test Bot]:</b>\n\n" +
...
function sendExpenseGroupNotification(chatId, exp) {
  ...
  var msg = "💸 <b>New expense added! [Test Bot]</b>\n" +
            "━━━━━━━━━━━━━\n" +
...
function sendExpenseUpdatedGroupNotification(chatId, exp) {
  ...
  var msg = "✏️ <b>Expense updated! [Test Bot]</b>\n" +
            "━━━━━━━━━━━━━\n" +
            "🏷️ <b>Item</b>: " + escapeHtml(description) + "\n" +
            "💰 <b>Total</b>: <b>" + amountFormatted + "</b>\n" +
            "👤 <b>Paid By</b>: <b>" + escapeHtml(paidBy) + "</b>\n" +
...
function sendSettlementGroupNotification(chatId, settle) {
  ...
  var msg = "🤝 <b>Settlement recorded! [Test Bot]</b>\n" +
            "━━━━━━━━━━━━━\n" +
...
function getGroupBalanceTextSummary(chatId, isDetailedSummary, passedGroupTitle) {
  ...
  if (expenses.length === 0 && settlements.length === 0) {
    var emptyHeader = groupTitle 
      ? "📊 <b>" + escapeHtml(groupTitle) + " — Expense Summary [Test]</b>\n━━━━━━━━━━━━━\n"
      : "📊 <b>splitnest Group Expense Summary [Test]</b>\n━━━━━━━━━━━━━\n";
  ...
  var header = groupTitle 
    ? "📊 <b>" + escapeHtml(groupTitle) + " — Balance Summary [Test]</b>\n━━━━━━━━━━━━━\n"
    : "📊 <b>splitnest Group Balance Summary [Test]</b>\n━━━━━━━━━━━━━\n";
...
function getAppReplyMarkup(chatId) {
  var cleanChatId = normalizeChatId(chatId);
  var appUrl = MINI_APP_URL.indexOf("http") === 0 ? MINI_APP_URL : ("https://" + MINI_APP_URL);
  if (cleanChatId) {
    var param = cleanChatId.replace(/^-/, "g_");
    appUrl = appUrl + (appUrl.indexOf("?") === -1 ? "?" : "&") + "startapp=" + encodeURIComponent(param);
  }
  return {
