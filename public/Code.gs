 ...
 * Telegram Bot: @splitnest_bot
 * Mini App URL: https://t.me/splitnest_bot/ambugan
 * Google Sheet ID: 106hKhXEEObyEbWJDxu0dFax-fKUIiDmkO1klpPPSJuM
 * 
 * Features:
...
// ==============================================================================
var TELEGRAM_BOT_TOKEN = "8949508191:AAGoH4SOKWp3StSyE1Es1XL8l3BR1CJkBpg";
...
// ==============================================================================
// 3. GEMINI AI OCR RECEIPT SCANNING & NATURAL LANGUAGE EXPENSE PARSER
// ==============================================================================

function callGeminiGenerateContent(apiKey, parts, responseMimeType) {
  var models = ["gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash", "gemini-flash-latest"];
  var lastError = "No response from Gemini API";
    ...
    try {
      ...
      if (resCode === 200) {
        ...
        if (json.candidates && json.candidates[0] && json.candidates[0].content && json.candidates[0].content.parts) {
          ...
          return { ok: true, data: parsed, modelUsed: modelName };
        }
      } else {
        lastError = "HTTP " + resCode + " (" + modelName + "): " + resText.slice(0, 150);
      }
    } catch (err) {
...
function scanReceiptWithGemini(base64Image, mimeType) {
  ...
  return callGeminiGenerateContent(apiKey, parts, "application/json");
}

/**
 * Natural language expense prompt parser using Gemini AI
 * Extracts expense parameters from chat messages like "Kate paid 500 for dinner split with Alex"
 */
function parseExpensePromptWithGemini(text, senderName, knownMembers) {
  var apiKey = GEMINI_API_KEY;
  if (!apiKey || apiKey === "YOUR_GEMINI_API_KEY_HERE") {
    return { ok: false, error: "GEMINI_API_KEY is not configured in Code.gs" };
  }

  var memberNames = Array.isArray(knownMembers) ? knownMembers.map(function(m){
    return (typeof m === 'string') ? m : (m.firstName || m.username || m.name || '');
  }).filter(Boolean) : [];

  var prompt = "You are an intelligent financial assistant for Splitnest, a shared expense tracker bot on Telegram.\n" +
    "A user sent this message in a group chat: \"" + String(text).replace(/"/g, '\\"') + "\"\n" +
    "Sender name: " + (senderName || "Unknown") + "\n" +
    "Known group members: " + JSON.stringify(memberNames) + "\n\n" +
    "Analyze if this message describes an expense, spending, purchase, bill, or money transaction.\n" +
    "If it is NOT an expense (e.g. casual greeting, chat, question, random words), return JSON:\n" +
    "{\n  \"isExpense\": false\n}\n\n" +
    "If it IS an expense (e.g. 'Dinner 1500', 'Paid 450 for coffee', 'Groceries 800 split equal', 'Grab taxi 320 paid by Kate'), extract and return JSON:\n" +
    "{\n" +
    '  "isExpense": true,\n' +
    '  "description": "Short name or item (e.g. Dinner, Coffee, Groceries, Grab)",\n' +
    '  "amount": 123.45,\n' +
    '  "currency": "₱",\n' +
    '  "paidBy": "Name of payer (pick best match from known members or sender)",\n' +
    '  "category": "Food & Drink | Groceries | Shopping | Entertainment | Transport | Utilities | General",\n' +
    '  "splitMode": "Equal"\n' +
    "}\n" +
    "Rules:\n" +
    "1. If no specific payer is named, assume the message sender paid.\n" +
    "2. Default currency is ₱ (Philippine Peso) unless another currency symbol ($, €, ¥, etc.) is specified.\n" +
    "3. Amount MUST be a positive number.\n" +
    "4. Return ONLY valid raw JSON with NO markdown code fences.";
...
function handleTelegramUpdate(update) {
  ...
  // 1. My Chat Member updates (bot added or removed as member/admin)
  if (update.my_chat_member) {
    ...
    if (!wasInGroup && isNowInGroup && !alreadyWelcomed) {
      ...
      var welcomeText = "👋 <b>Welcome to splitnest!</b>\n\n" +
                        ...
                        "• 💬 <b>Natural Prompts</b>: Type expenses like <i>\"Kate paid 450 for coffee\"</i> or <i>\"Dinner 1200\"</i>\n" +
                        "• 📊 <b>Commands</b>: <code>/balance</code>, <code>/summary</code>, <code>/help</code>\n\n" +
  ...
  // 2. Register active sender
  if (msg.from && !msg.from.is_bot && cleanChatId) {
    try { registerUsersBatch(cleanChatId, [msg.from]); } catch (e) {}
  }

  // 3. Handle new_chat_members (when bot or other users join the group)
  if (msg.new_chat_members && Array.isArray(msg.new_chat_members) && msg.new_chat_members.length > 0) {
    try {
      var humanMembers = msg.new_chat_members.filter(function(u){ return !u.is_bot; });
      if (humanMembers.length > 0) {
        registerUsersBatch(cleanChatId, humanMembers);
      }
    } catch (e) {}

    var hasBot = msg.new_chat_members.some(function(u) {
      return u.is_bot && (u.username && u.username.toLowerCase().indexOf("nest") !== -1);
    });
    ...
    if (hasBot || !alreadyWelcomed) {
      try { scriptProps.setProperty(welcomePropKey, "true"); } catch(e){}
      var welcomeText = "👋 <b>Welcome to splitnest!</b>\n\n" +
                        "Split bills, track shared expenses, and scan receipts with Gemini AI.\n\n" +
                        "⚡ <b>Quick Guide:</b>\n" +
                        "• 📸 <b>Send a receipt photo</b> to scan & split line items instantly\n" +
                        "• 💬 <b>Type any expense</b> (e.g. <i>\"Kate paid 450 for coffee\"</i>)\n" +
                        "• 📊 Type <code>/balance</code> to view net balances\n\n" +
                        "Tap below to open your group's shared ledger!";
      sendTelegramMessage(chatId, welcomeText, getAppReplyMarkup(chatId));
      return createJsonResponse({ ok: true, status: "new_members_welcomed" });
    }
  }
  ...
  if (commandOnly === "/start" || commandOnly === "!start") {
    var startReply = "✨ <b>Welcome to splitnest!</b>\n\n" +
                     "Split bills, track shared expenses, and scan receipts with Gemini AI.\n\n" +
                     "📸 <b>Receipt OCR</b>: Send any photo of a bill to parse items & totals!\n\n" +
                     "💬 <b>Natural Prompts</b>: Type expenses directly (e.g. <i>\"Kate paid 500 for groceries\"</i>)\n\n" +
  ...
  if (commandOnly === "/help" || commandOnly === "!help" || lowerText === "help") {
    var helpText = "📖 <b>splitnest Bot Commands:</b>\n\n" +
                   ...
                   "• 💬 <b>Type any expense</b> - <i>\"Kate paid 450 for coffee\"</i> or <i>\"Dinner 1200\"</i>\n" +
  ...
  // ----------------------------------------------------------------------------
  // NATURAL LANGUAGE EXPENSE PROMPT PROCESSING (GEMINI AI)
  // ----------------------------------------------------------------------------
  var isExplicitExpenseCmd = (commandOnly === "/add" || commandOnly === "/expense" || commandOnly === "/spend" || commandOnly === "/split");
  var containsNumbers = /\d+/.test(rawText);

  if (isExplicitExpenseCmd || containsNumbers) {
    try {
      var senderName = (msg.from && (msg.from.first_name || msg.from.username)) ? (msg.from.first_name || msg.from.username) : "Member";
      var promptText = isExplicitExpenseCmd ? rawText.replace(/^\/\w+(@\w+)?\s*/i, "") : rawText;
      
      if (promptText.trim().length > 1) {
        var groupUsers = getUsers(cleanChatId);
        var aiParsed = parseExpensePromptWithGemini(promptText, senderName, groupUsers);

        if (aiParsed && aiParsed.ok && aiParsed.data && aiParsed.data.isExpense && Number(aiParsed.data.amount) > 0) {
          var expData = aiParsed.data;
          var expDesc = expData.description || "Expense";
          var expAmt = Number(expData.amount) || 0;
          var expCurr = expData.currency || "₱";
          var expPaidBy = expData.paidBy || senderName;
          var expCat = expData.category || "General";
          var expMode = expData.splitMode || "Equal";

          // Auto-record to Expenses Sheet
          var newId = "EXP-" + Utilities.formatDate(new Date(), "GMT+8", "yyyyMMdd-HHmmss");
          ...
          expSheet.appendRow([
            ...
            0,
            0,
            senderName + " (Telegram Prompt)",
            expCat,
            cleanChatId,
            ""
          ]);

          var confirmMsg = "✨ <b>Expense Logged via AI!</b>\n" +
                           "━━━━━━━━━━━━━\n" +
                           "📝 <b>Description</b>: <b>" + escapeHtml(expDesc) + "</b>\n" +
                           "💰 <b>Amount</b>: <b>" + expCurr + expAmt.toFixed(2) + "</b>\n" +
                           "👤 <b>Paid By</b>: <b>" + escapeHtml(expPaidBy) + "</b>\n" +
                           "🏷️ <b>Category</b>: " + escapeHtml(expCat) + "\n" +
                           "👥 <b>Split</b>: " + escapeHtml(expMode) + " across group\n" +
                           "━━━━━━━━━━━━━\n" +
                           "✅ <i>Saved to Google Sheets database!</i>";

          sendTelegramMessage(chatId, confirmMsg, getAppReplyMarkup(chatId));
          return createJsonResponse({ ok: true, status: "prompt_expense_logged", expense: expData });
        }
      }
    } catch (nlpErr) {
      Logger.log("NLP Expense Prompt error: " + nlpErr.toString());
    }
  }
